...
fn test_demonstrate_timezone_inconsistency_issue() {
    ...
    assert!((progress - 50.0).abs() < 0.01, "Expected 50% progress, got {:.2}%", progress);
}
...
fn test_current_behavior_with_future_times() {
    ...
    if start_time > current_time {
        assert_eq!(progress, 0.0, "Progress should be 0% when start time is in the future");
    }
}
...
fn test_timezone_consistency_fix_verification() {
    ...
    assert!(diff.num_seconds() < 5, "get_current_time() calls should be consistent");
    ...
    assert!((diff.num_seconds() - 60).abs() < 5, "Difference should be ~60 seconds");
    ...
    assert!((progress - 50.0).abs() < 0.1, "Progress should be ~50% at midpoint");
