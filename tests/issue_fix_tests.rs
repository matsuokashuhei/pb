...
mod issue_fix_tests {
    ...
    fn test_issue_1_base_time_calculation_fix() {
        ...
        let start_time =
            NaiveDateTime::parse_from_str("2025-01-27 14:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
        ...
        let expected_2h =
            NaiveDateTime::parse_from_str("2025-01-27 16:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
        assert_eq!(
            end_time_2h, expected_2h,
            "2h should be 2 hours after start time"
        );

        // Example 2: "90m" should add 90 minutes to start time
        let end_time_90m = parse_time_with_base("90m", Some(start_time)).unwrap();
        let expected_90m =
            NaiveDateTime::parse_from_str("2025-01-27 15:30:00", "%Y-%m-%d %H:%M:%S").unwrap();
        assert_eq!(
            end_time_90m, expected_90m,
            "90m should be 90 minutes after start time"
        );
        ...
        let expected_3600s =
            NaiveDateTime::parse_from_str("2025-01-27 15:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
        assert_eq!(
            end_time_3600s, expected_3600s,
            "3600s should be 3600 seconds (1 hour) after start time"
        );

        // Example 3: "7d" should add 7 days to start time (date-only start)
        let start_date =
            NaiveDateTime::parse_from_str("2025-01-27 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
        let end_time_7d = parse_time_with_base("7d", Some(start_date)).unwrap();
        let expected_7d =
            NaiveDateTime::parse_from_str("2025-02-03 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap();
        assert_eq!(
            end_time_7d, expected_7d,
            "7d should be 7 days after start date"
        );
    }
    ...
    fn test_issue_2_seconds_support_fix() {
        ...
        let expected_30s =
            NaiveDateTime::parse_from_str("2025-01-27 14:00:30", "%Y-%m-%d %H:%M:%S").unwrap();
        assert_eq!(
            result_30s.unwrap(),
            expected_30s,
            "30s should add 30 seconds to base time"
        );
        ...
        assert_eq!(
            result_3600s.unwrap(),
            expected_3600s,
            "3600s should add 3600 seconds (1 hour) to base time"
        );
    ...
    fn test_backward_compatibility() {
        ...
        assert!(
            datetime_result.is_ok(),
            "DateTime parsing should still work"
        );

        // Relative time parsing should still use current time when no base is provided
        let relative_result = parse_time("2h");
        assert!(
            relative_result.is_ok(),
            "Relative time parsing should still work"
        );
        ...
        assert!(
            with_base_result.is_ok(),
            "Relative time with base should work"
        );

        // They should be different (unless current time happens to match the base, which is extremely unlikely)
        assert_ne!(
            relative_result.unwrap(),
            with_base_result.unwrap(),
    ...
    fn test_edge_cases_for_fixes() {
        ...
        let result_86400s = parse_time_with_base("86400s", Some(base_time));
        assert!(
            result_86400s.is_ok(),
            "86400s should be valid (maximum - 1 day)"
        );

        // Test that values over the limit are rejected
        let result_86401s = parse_time_with_base("86401s", Some(base_time));
        assert!(
            result_86401s.is_err(),
            "86401s should be invalid (over limit)"
        );
