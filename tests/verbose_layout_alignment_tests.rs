...
fn test_verbose_layout_exact_format() {
    ...
    assert_eq!(lines[0].len(), 40, "Date line should be exactly 40 characters");
    
    // Verify line 2: progress bar with no leading spaces
    assert!(lines[1].starts_with('█'), "Progress bar should start with filled character");
    assert_eq!(lines[1].chars().count(), 40, "Progress bar should be exactly 40 visual characters");
    assert_eq!(lines[1].chars().take_while(|&c| c == ' ').count(), 0, "Progress bar should have no leading spaces");
    
    // Verify line 3: statistics with no leading spaces
    assert!(lines[2].starts_with("2.5%"), "Statistics should start with percentage");
    assert!(lines[2].contains("elapsed"), "Statistics should contain 'elapsed'");
    assert!(lines[2].contains("remaining"), "Statistics should contain 'remaining'");
    assert_eq!(lines[2].chars().take_while(|&c| c == ' ').count(), 0, "Statistics should have no leading spaces");
}
...
fn test_verbose_layout_alignment_consistency() {
    ...
    let test_cases = vec![
        ("2025-01-01 00:00:00", "2025-12-31 00:00:00", "2025-06-15 00:00:00", 50.0),
        ("2025-08-02 00:00:00", "2025-08-31 00:00:00", "2025-08-02 18:00:00", 2.5),
        ("2025-01-01 10:00:00", "2025-01-01 20:00:00", "2025-01-01 15:00:00", 50.0),
    ];
    ...
    for (start_str, end_str, current_str, percentage) in test_cases {
        ...
        assert_eq!(lines[0].len(), 40, "Date line should be 40 characters in scenario ({}, {}, {}, {})", start_str, end_str, current_str, percentage);
        
        // Progress bar should be exactly 40 visual characters
        assert_eq!(lines[1].chars().count(), 40, "Progress bar should be 40 visual characters in scenario ({}, {}, {}, {})", start_str, end_str, current_str, percentage);
    }
}